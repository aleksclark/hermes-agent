...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_thread_command(self, event: MessageEvent) -> str:
        """Create, list, or close real Telegram forum topics."""
        source = event.source
        chat_id = source.chat_id
        args = event.get_command_args().strip()
        ...
        if parts[0].lower() == "close":
            ...
            try:
                ...
                return f"Failed to close topic **{name}**: {exc}"
        ...
        if existing_id is not None:
            return (
                ...
                "Reply in the topic to continue chatting."
            )
        ...
        try:
            ...
            if (
                ...
                if source.chat_type == "dm":
                    return (
                        "This DM doesn't have topics enabled. The user needs Telegram Premium "
                        "and must enable Topics in this chat (tap bot name → Topics)."
                    )
                return (
                    "This chat doesn't support forum topics. Enable Topics in the group settings "
                    "(the group must be a supergroup)."
                )
            return f"Failed to create forum topic: {exc}"

        self._thread_store.add(chat_id, name, topic_id)
        if hasattr(adapter, "_dm_topics"):
            adapter._dm_topics[f"{chat_id}:{name}"] = topic_id
        try:
            await adapter.send(
                chat_id=chat_id,
                content=f"🧵 Thread **{name}** is ready. Messages here have their own conversation.",
                metadata={"thread_id": str(topic_id)},
            )
        except Exception as exc:
            logger.warning("Failed to send initial message to new topic %s: %s", name, exc)

        return f"🧵 Forum topic **{name}** created! Reply in the new topic to start chatting."
