...
Route definitions require `provider` and `model`. The conservative optional fields are `reasoning_effort`, `base_url`, `api_mode`, `max_output_tokens`, and `request_overrides`. `reasoning_effort` accepts the normal Hermes levels (`none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`) and applies only to the selected child. Credentials are deliberately forbidden in aliases: Hermes resolves the named provider through the same operator-owned credential store used by normal CLI/gateway startup. `api_key`, tokens, subprocess commands, and raw model-facing provider/model/base URL fields are not accepted.
